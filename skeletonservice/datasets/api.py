...
class SkeletonResource__gen_skeletons_via_msg_B(Resource):
    ...
    def process(datastack_name: str, skvn: int, rid: int):
        ...
        current_app.logger.info(f"SkeletonService sending payload for rid {rid} to exchange {exchange}")
        c.publish(exchange, payload, attributes)

        current_app.logger.info(f"Message has been dispatched to {exchange}: {datastack_name} {rid} skvn:{skvn} {current_app.config['SKELETON_CACHE_BUCKET']}")
